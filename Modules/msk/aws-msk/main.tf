...
resource "aws_security_group" "msk_self_sg" {
  count = var.create ? 1 : 0
  name        = "${var.name}-self"
  description = "Security group for MSK Cluster"
  vpc_id      = var.broker_vpc_id
  tags = merge(
    var.tags,
    {
      Name = "${var.name}-self"
...
resource "aws_security_group_rule" "allow_self_ingress" {
  ...
  to_port                  = 65535
  protocol                 = "-1"
  security_group_id        = aws_security_group.msk_self_sg[0].id
  source_security_group_id = aws_security_group.msk_self_sg[0].id
...
resource "aws_msk_cluster" "this" {
  ...
  broker_node_group_info {
    ...
    security_groups = compact(concat(var.broker_node_security_groups, [aws_security_group.msk_self_sg[0].id]))
