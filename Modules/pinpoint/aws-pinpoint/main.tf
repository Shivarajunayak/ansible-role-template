# data "aws_secretsmanager_secret" "gcm_token_secret" {
#   name = var.pinpoint_gcm_token_secret_name # Replace with your actual secret name
# }

# data "aws_secretsmanager_secret_version" "gcm_token_secret_current" {
#   # secret_id = data.aws_secretsmanager_secret.gcm_token_secret.id
#   secret_id = try(data.aws_secretsmanager_secret.gcm_token_secret.id, null) 
# }


data "aws_secretsmanager_secret" "gcm_token_secret" {
  count = var.gcm != null ? 1 : 0
  name  = var.gcm.secret_name
}

data "aws_secretsmanager_secret_version" "gcm_token_secret_current" {
  count     = var.gcm != null ? 1 : 0
  secret_id = data.aws_secretsmanager_secret.gcm_token_secret[0].id
}


resource "aws_iam_role" "this" {
  name               = "${var.name}-pinpoint-role"
...
#   count          = null != var.gcm ? 1 : 0
#   application_id = aws_pinpoint_app.this.application_id
#   default_authentication_method = var.gcm.default_authentication_method
#   service_json                  = var.gcm != null && length(data.aws_secretsmanager_secret_version.gcm_token_secret_current) > 0 ? data.aws_secretsmanager_secret_version.gcm_token_secret_current.secret_string : ""
# }
...
resource "aws_pinpoint_gcm_channel" "gcm" {
  ...
  application_id                = aws_pinpoint_app.this.application_id
  default_authentication_method = var.gcm.default_authentication_method
  service_json                  = data.aws_secretsmanager_secret_version.gcm_token_secret_current[0].secret_string
}
