...
resource "aws_opensearch_domain" "this" {
  ...
  dynamic "advanced_security_options" {
    ...
    content {
      anonymous_auth_enabled         = try(advanced_security_options.value.anonymous_auth_enabled, true)
      ...
      dynamic "master_user_options" {
        ...
        content {
          ...
          master_user_name     = try(local.secret["opensearch.net.http.auth.user"], "Opensearch${local.environment}user")
          master_user_password = try(local.secret["opensearch.net.http.auth.pass"], "Opensearchhmcl${local.environment}password777!")
        }
      }
    }
  }
