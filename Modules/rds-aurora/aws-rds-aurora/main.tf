...
resource "aws_security_group" "rds_self_sg" {

  count = local.create ? 1 : 0

  name        = "${local.security_group_name}-self"
...
resource "aws_security_group_rule" "allow_self_ingress" {
  ...
  to_port                  = 65535
  protocol                 = "-1"
  security_group_id        = aws_security_group.rds_self_sg[0].id
  source_security_group_id = aws_security_group.rds_self_sg[0].id
...
resource "aws_rds_cluster" "this" {
  ...
  vpc_security_group_ids = compact(concat([try(aws_security_group.this[0].id, "")], var.vpc_security_group_ids, [aws_security_group.rds_self_sg[0].id]))
